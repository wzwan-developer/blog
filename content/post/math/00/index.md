...
Hence $\mathrm{det}(S)$ must be equal to 1 when $\mathrm{det}(AB^T)> 0$, and -1 when $\mathrm{det}(AB^T)< 0$,and -1 when $\mathrm{det}(AB^T)< 0$.<br> &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Next, extremum values od $\left \| A-RB^2 \right \|$ is derived as follows: from (11) we have $$\begin{aligned}
